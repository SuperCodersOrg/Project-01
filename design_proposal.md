...
|                  | `insert(index)`    | **O(1)**  | **O(n)**           | **O(n)**   | Insertion at the end requires no shifting, while insertion elsewhere shifts subsequent elements.                                |
...
The average-case complexity of the HashMap assumes a well-distributed hash function and a maintained load factor below the rehash threshold. Similarly, the amortized complexity of `DynamicArray::append()` is achieved through exponential capacity growth, making expensive resize operations infrequent relative to the total number of insertions.
